...
function init(){
	...
	$("#enfermedad_form").on("submit",function(e)
	{
...
function listar()
{
	tabla=$('#enfermedad_data').dataTable(
	{
		...
		"ajax":
				{
					url: '../ajax/enfermedad.php?op=listar',
					type : "get",
					dataType : "json",						
					error: function(e){
...
{
	$.post("../ajax/enfermedad.php?op=mostrar",{id_enfermedad : id_enfermedad}, function(data, status)
	{
		data = JSON.parse(data);
		 ...
		 //alert(data.cedula);
				...
				$('.modal-title').text("Editar Enfermedad");
				$('#id_enfermedad').val(id_enfermedad);
...
function guardaryeditar(e)
{
	...
	var formData = new FormData($("#enfermedad_form")[0]);


		$.ajax({
			url: "../ajax/enfermedad.php?op=guardaryeditar",
		    type: "POST",
		    data: formData,
		    contentType: false,
		    processData: false,
		    ...
		    {                    
	            ...
	            $('#enfermedad_form')[0].reset();
				$('#enfermedadModal').modal('hide');

				$('#resultados_ajax').html(datos);
				$('#enfermedad_data').DataTable().ajax.reload();
