...
function init(){
	...
	$("#add_button").click(function(){
			...
			$(".modal-title").text("Agregar Trabajo");
...
function listar()
{
	tabla=$('#trabajo_data').dataTable(
	{
		...
		"ajax":
				{
					url: '../ajax/trabajo.php?op=listar',
					type : "get",
					dataType : "json",						
					error: function(e){
...
{
	$.post("../ajax/trabajo.php?op=mostrar",{id_trabajo : id_trabajo}, function(data, status)
	{
		data = JSON.parse(data);
		 ...
		 //alert(data.cedula);
				...
				$('.modal-title').text("Editar Trabajo");
...
function guardaryeditar(e)
{
	...
	var formData = new FormData($("#trabajo_form")[0]);


		$.ajax({
			url: "../ajax/trabajo.php?op=guardaryeditar",
		    type: "POST",
		    data: formData,
		    contentType: false,
		    processData: false,
		    ...
		    {                    
	            ...
	            $('#trabajo_form')[0].reset();
				$('#trabajoModal').modal('hide');

				$('#resultados_ajax').html(datos);
				$('#trabajo_data').DataTable().ajax.reload();
